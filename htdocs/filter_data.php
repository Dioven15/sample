...
function filter_global($global){
	foreach ($global as $key => $value) {
		$global[$key] = ldap_filter($value);
	}
}

function ldap_filter($string){
	if(is_string($string)){
		$string = filter_data($string);
		$string = ldap_escape(htmlspecialchars($string),null,LDAP_ESCAPE_FILTER);
	}
	return $string;
}
...
$glob_var = array($_GET,$_POST,$_REQUEST);
filter_global($glob_var);
